<table>
    <thead>
        ...
        <tr>
            <th style="font-weight: bold; text-align: center; font-size: 12px;" colspan="6">LAPORAN REKAP PENCAIRAN</th>
        </tr>
        <tr>
    ...
    <tbody>
        @php $no = 1 @endphp
        @foreach($pencairan as $pnc)
        <tr>
            <td style="border: 1px solid #000; text-align: center;">{{ $no++ }}</td>
            <td style="border: 1px solid #000;">{{ $pnc->keterangan }}</td>
            <td style="border: 1px solid #000; text-align: center;">{{ $pnc->jumlah_anggota }}</td>
            <td style="border: 1px solid #000;">{{ $pnc->nominal }}</td>
            <td style="border: 1px solid #000;">{{ $pnc->persen_jumlah }}</td>
            <td style="border: 1px solid #000;">{{ $pnc->persen_nominal }}</td>
        </tr>
        @endforeach
        <tr>
