...
class KecamatanController extends Controller
{
    ...
    public function read(Request $request)
    {
        ...
        $read = KopKecamatan::select('*')->orderBy($sortBy, $sortDir);
        ...
        if ($search != NULL) {
            $read->whereRaw("(kode_kecamatan LIKE '%" . $search . "%' OR nama_kecamatan LIKE '%" . $search . "%')");
        }
        ...
        if ($search || $id_cabang || $type) {
            $total = KopKecamatan::orderBy($sortBy, $sortDir);

            if ($search) {
                $total->whereRaw("(kode_kecamatan LIKE '%" . $search . "%' OR nama_kecamatan LIKE '%" . $search . "%')");
            }

            $total = $total->count();
        } else {
            $total = KopKecamatan::all()->count();
        }
