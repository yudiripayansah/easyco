        ...
        <b-row>
          <b-col cols="6">
            <b-form-group label="Jumlah Hari 1" label-for="jumlah_hari_1">
              <b-form-input
                id="jumlah_hari_1"
                v-model="form.data.jumlah_hari_1"
          ...
          <b-col cols="6">
            <b-form-group label="Jumlah Hari 2" label-for="jumlah_hari_2">
              <b-form-input
                id="jumlah_hari_2"
                v-model="$v.form.data.jumlah_hari_2.$model"
          ...
          <b-col cols="6">
            <b-form-group label="Kategori Par" label-for="kategori_par">
              <b-form-input
                id="kategori_par"
                v-model="$v.form.data.kategori_par.$model"
          ...
          <b-col cols="6">
            <b-form-group label="Cpp" label-for="cpp">
              <b-form-input
                id="cpp"
                v-model="$v.form.data.cpp.$model"
                :state="validateState('cpp')"
              />
            </b-form-group>
          </b-col>
...
export default {
  ...
  methods: {
    ...
    async doGet() {
      ...
      try {
        let req = await easycoApi.katgoriparRead(payload, this.user.token);
    ...
    async doSave(e) {
      ...
      if (!this.$v.form.$anyError) {
        ...
        try {
          ...
          let req = false;
          if (payload.id) {
            req = await easycoApi.katgoriparUpdate(payload, this.user.token);
          } else {
            req = await easycoApi.katgoriparCreate(payload, this.user.token);
          }
          let { status } = req.data;
          if (status) {
    ...
    async doUpdate(item) {
      try {
        let req = await easycoApi.katgoriparDetail(
          `id=${item.id}`,
          this.user.token
        );
    ...
    async doDelete(item, prompt) {
      if (prompt) {
        ...
        try {
          let req = await easycoApi.katgoriparDelete(
