...
export default {
  ...
  methods: {
    ...
    async doSave(e) {
      ...
      if (!this.$v.form.$anyError) {
        ...
        try {
          ...
          let req = false
          if(payload.id) {
            req = await easycoApi.desaUpdate(payload,this.user.token)
          } else {
            req = await easycoApi.desaCreate(payload,this.user.token)
          }
          let { status } = req.data
          if(status) {
    ...
    async doUpdate(item) {
      try {
        let req = await easycoApi.desaDetail(`id=${item.id}`,this.user.token)
        let { data, status, msg } = req.data
        if(status) {
          this.form.data = data
    ...
    async doDelete(item,prompt) {
      if(prompt){
        ...
        try {
          let req = await easycoApi.desaDelete(`id=${this.remove.data.id}`,this.user.token)
          let { status } = req.data
          if(status) {
