<template>
  <div>
      ...
      <b-row no-gutters>
        ...
        <b-col cols="12">
          <b-table responsive bordered outlined small striped hover :fields="table.fields" :items="table.items"
            :sort-by.sync="paging.sortBy" :sort-desc.sync="paging.sortDesc" show-empty 
    ...
    <b-modal title="Form Pengajuan Pembiayaan" id="modal-form" hide-footer size="lg" centered>
        ...
        <b-row>
          ...
          <b-col cols="12" sm="6">
            <b-form-group label="Petugas">
              <b-select v-model="form.data.kode_petugas" :options="opt.petugas" />
            </b-form-group>
          </b-col>
          <b-col cols="12" sm="6">
          ...
          <b-col cols="12" sm="4">
            <b-form-group label="Peruntukan">
              <b-select v-model="form.data.peruntukan" :options="opt.peruntukan"/>
            </b-form-group>
          </b-col>
          <b-col cols="12" sm="12">
          ...
          <b-col cols="12" sm="8">
            <b-form-group label="Rencana Jangka Waktu">
              <b-row>
                ...
                <b-col cols="8">
                  <b-select v-model="form.data.rencana_periode_jwaktu" :options="opt.rencanaPeriodeJangkaWaktu"/>
                </b-col>
              </b-row>
            </b-form-group>
          </b-col>
          <b-col cols="12" sm="4">
            <b-form-group label="Jenis Pembiayaan">
              <b-select v-model="form.data.jenis_pembiayaan" :options="opt.jenisPembiayaan"/>
            </b-form-group>
          </b-col>
          <b-col cols="12" sm="4">
            <b-form-group label="Sumber Pengembalian">
              <b-select v-model="form.data.sumber_pengembalian" :options="opt.sumberPengembalian"/>
            </b-form-group>
          </b-col>
          <b-col cols="12" sm="4">
            <b-form-group label="Tanggal Pengajuan">
              <b-form-datepicker v-model="form.data.tanggal_pengajuan"
                :date-format-options="{ year: 'numeric', month: 'numeric', day: 'numeric' }" locale="id" @input="setTanggalCair()"/>
            </b-form-group>
          </b-col>
          <b-col cols="12" sm="4">
            <b-form-group label="Tanggal Pencairan">
              <b-form-datepicker v-model="form.data.rencana_droping"
          ...
          <b-col cols="12" sm="6">
            <b-form-group label="TTD Anggota" description="Click to upload">
              <label for="fm-ttd_anggota" class="w-100" ref="previewImage">
                <b-img-lazy :src="(form.data.ttd_anggota) ? form.data.ttd_anggota : '/media/doc-upload.png'" fluid
                  class="w-100" />
                <input type="file" ref="fm-ttd_anggota" hidden id="fm-ttd_anggota"
                  @change="previewImage($event, 'ttd_anggota')" accept="image/*">
              </label>
            </b-form-group>
          </b-col>
          <b-col cols="12" sm="6">
            <b-form-group label="TTD Ketua Rembug" description="Click to upload">
              <label for="fm-ttd_ketua_majelis" class="w-100" ref="previewImage">
                <b-img-lazy :src="(form.data.ttd_ketua_majelis) ? form.data.ttd_ketua_majelis : '/media/doc-upload.png'"
                  fluid class="w-100" />
                <input type="file" ref="fm-ttd_ketua_majelis" hidden id="fm-ttd_ketua_majelis"
                  @change="previewImage($event, 'ttd_ketua_majelis')" accept="image/*">
              </label>
            </b-form-group>
          </b-col>
          <b-col cols="12" sm="6">
            <b-form-group label="TTD TPL" description="Click to upload">
              <label for="fm-ttd_tpl" class="w-100" ref="previewImage">
                <b-img-lazy :src="(form.data.ttd_tpl) ? form.data.ttd_tpl : '/media/doc-upload.png'" fluid
                  class="w-100" />
                <input type="file" ref="fm-ttd_tpl" hidden id="fm-ttd_tpl" @change="previewImage($event, 'ttd_tpl')"
                  accept="image/*">
              </label>
            </b-form-group>
          </b-col>
          <b-col cols="12" sm="6">
            <b-form-group label="TTD Pasangan" description="Click to upload">
              <label for="fm-ttd_suami" class="w-100" ref="previewImage">
                <b-img-lazy :src="(form.data.ttd_suami) ? form.data.ttd_suami : '/media/doc-upload.png'" fluid
                  class="w-100" />
                <input type="file" ref="fm-ttd_suami" hidden id="fm-ttd_suami"
                  @change="previewImage($event, 'ttd_suami')" accept="image/*">
              </label>
            </b-form-group>
          </b-col>
          <b-col cols="12" sm="4">
            <b-form-group label="Dokumen KTP" description="Click to upload">
              <label for="fm-doc_ktp" class="w-100" ref="previewImage">
                <b-img-lazy :src="(form.data.doc_ktp) ? form.data.doc_ktp : '/media/doc-upload.png'" fluid
                  class="w-100" />
                <input type="file" ref="fm-doc_ktp" hidden id="fm-doc_ktp" @change="previewImage($event, 'doc_ktp')"
                  accept="image/*">
              </label>
            </b-form-group>
          </b-col>
          <b-col cols="12" sm="4">
            <b-form-group label="Dokumen Kartu Keluarga" description="Click to upload">
              <label for="fm-doc_kk" class="w-100" ref="previewImage">
                <b-img-lazy :src="(form.data.doc_kk) ? form.data.doc_kk : '/media/doc-upload.png'" fluid
                  class="w-100" />
                <input type="file" ref="fm-doc_kk" hidden id="fm-doc_kk" @change="previewImage($event, 'doc_kk')"
                  accept="image/*">
              </label>
            </b-form-group>
          </b-col>
          <b-col cols="12" sm="4">
            <b-form-group label="Dokumen Pendukung" description="Click to upload">
              <label for="fm-doc_pendukung" class="w-100" ref="previewImage">
                <b-img-lazy :src="(form.data.doc_pendukung) ? form.data.doc_pendukung : '/media/doc-upload.png'" fluid
                  class="w-100" />
                <input type="file" ref="fm-doc_pendukung" hidden id="fm-doc_pendukung"
                  @change="previewImage($event, 'doc_pendukung')" accept="image/*">
              </label>
            </b-form-group>
          </b-col>
        </b-row>
...
export default {
  ...
  watch: {
    ...
    form: {
      handler(val) {
        ...
        if (noAnggota) {
          this.anggota = this.opt.anggota.find((i) => i.value == noAnggota).data
        }
      },
      deep: true
    }
  ...
  methods: {
    ...
    async doGetAnggota() {
      ...
      try {
        ...
        if (status) {
          this.opt.anggota = []
          data.map((item) => {
            item.no_anggota = Number(item.no_anggota)
    ...
    async doSave() {
      ...
      try {
        ...
        if (payload.id) {
          ...
          req = await easycoApi.pengajuanPembiayaanCreate(payload, this.user.token)
        }
        let { data, status, msg } = req.data
        if (status) {
          this.doGet()
    ...
    async doDelete(data, dialog) {
      if (dialog) {
        ...
        try {
          ...
          let req = await easycoApi.pengajuanDelete(`?id=${this.remove.data.id}`, this.user.token)
          let { data, status, msg } = req.data
          if (status) {
    ...
    setTanggalCair(){
      ...
      this.form.data.rencana_droping = new Date(date.setDate(date.getDate() + 7));
    },
