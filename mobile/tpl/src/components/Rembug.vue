<template>
  <div>
    <v-select solo label="Rembug" class="mb-4" hide-details :items="rembug" v-model="list.rembug"
      @change="$emit('refreshAnggota', list.rembug, list.date)" />
    <v-menu ref="menu" v-model="dateShow" :close-on-content-click="false" :return-value.sync="list.date"
      transition="scale-transition" offset-y min-width="auto" class="white">
      <template v-slot:activator="{ on, attrs }">
        <v-text-field solo v-model="list.date" label="Tanggal" readonly v-bind="attrs"
          v-on="on"/>
      </template>
      <v-date-picker v-model="list.date" no-title scrollable>
        ...
        <v-btn text color="primary" @click="$refs.menu.save(list.date);$emit('refreshAnggota', list.rembug, list.date);">
          OK
        </v-btn>
      </v-date-picker>
    </v-menu>
    ...
    <v-container class="pa-0" v-if="list.anggota && list.anggota.length > 0">
      ...
      <v-card class="white elevation-3 rounded-lg pa-3 align-items-end mb-3" v-for="(agt, agtIndex) in list.anggota"
        :key="agtIndex">
        <v-container class="d-flex justify-space-between pa-0 flex-column">
          <h5 class="text-h6 font-weight-bold d-inline-flex justify-space-between align-center">{{ agt.nama_anggota }}
            <small class="text-caption grey--text ms-3">{{ agt.no_anggota }}</small></h5>
        ...
        <v-container class="pa-0 d-flex justify-space-between">
          <v-row class="justify-end">
            <v-col cols="4">
              <router-link :to="`/transaksi/setoran-form/${list.rembug}/${agt.no_anggota}/${list.date}`">
