            ...
            <v-row>
              <v-col cols="12">
                <v-text-field 
                  ...
                  hide-details
                  outlined
                  :value="thousandMask(form.data.saldo)"
                  label="Saldo"
                  disabled
                />
              </v-col>
...
export default {
  ...
  methods: {
    ...
    async getProduk() {
      ...
      this.opt.produk = []
      try {
        let req = await services.rekeningTabunganAnggota(payload, this.user.token)
        if(req.status === 200) {
          req.data.data.map((item) => {
    ...
    async doSave() {
      ...
      if(payloadData.id){
        ...
        try {
          let req = await services.tutupTabunganAnggota(payload, this.user.token);
          if (req.status === 200) {
            if(req.data.status){
              this.alert = {
                show: true,
                msg: "Registrasi Penciran Tabungan Berhasil",
              };
              setTimeout(() => {
            ...
            this.alert = {
              show: true,
              msg: "Registrasi Penciran Tabungan Gagal",
            };
          }
        } catch (error) {
          this.alert = {
